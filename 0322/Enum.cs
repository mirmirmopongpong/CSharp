...
namespace Basic
{
    class ConstAndReadonly
    {
        ...
        static void Main(string[] args)
        {
            ...
            foreach (var size in System.Enum.GetValues(typeof(Size)))
            {
                Console.WriteLine("{0,10} : {1:C}", size, price[(int)size]);
            }
            Console.WriteLine("\nColors Enum iteration");
            foreach (var color in System.Enum.GetValues(typeof(Colors)))
            {
                Console.WriteLine("{0,10} : {1}", color, Convert.ToInt32(color));
            }
            Console.WriteLine("\n커피 가격표(Enum iteration with value)");
            foreach (var coffee in System.Enum.GetValues(typeof(Coffee)))
            {
